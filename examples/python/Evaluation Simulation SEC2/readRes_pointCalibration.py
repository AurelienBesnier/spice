...
if __name__ == "__main__":
    ...
    for i in range(len(res_600_655)):
        for j in range(12):
            if (
                int(res_simulation["xSite"][i]) == xSite[j]
                and int(res_simulation["ySite"][i]) == ySite[j]
                and int(res_simulation["zSite"][i]) == zSite[j]
            ):
                print(
                    res_simulation["xSite"][i],
                    res_simulation["ySite"][i],
                    res_simulation["zSite"][i],
                    res_600_655[i],
                )
